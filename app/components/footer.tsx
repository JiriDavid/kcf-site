  ...
  return (
    <footer className="border-t border-white/10 bg-black/70 backdrop-blur-xl">
      <div className="container grid gap-8 sm:gap-10 py-8 sm:py-10 lg:grid-cols-12">
        <div className="lg:col-span-5 space-y-3 sm:space-y-4">
          ...
          <p className="text-sm text-white">
            A worship-inspired community practicing presence, building
            disciples, and sending leaders to every sphere.
          </p>
          ...
          <div className="flex flex-col gap-2 text-sm text-white">
            {[
              ...
              <Link
                key={href}
                href={href}
                className="transition hover:text-primary text-white py-1"
              >
                {label}
              </Link>
            ))}
      ...
      <div className="border-t border-white/5 py-4 text-center text-xs text-white">
        © {new Date().getFullYear()} KCF Fellowship. All rights reserved.
      </div>
    </footer>
  );
}
